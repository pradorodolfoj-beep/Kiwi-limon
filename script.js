...
const readProducts = () => {
  try {
    const saved = localStorage.getItem('kiwi-products');
    return saved ? JSON.parse(saved) : structuredClone(defaultProducts);
  } catch {
...
function closeLayer(id) {
  ...
  const anyOpen = ['cart-drawer', 'customization-modal', 'login-modal', 'product-modal'].some((layer) => !$(layer).classList.contains('hidden'));
  if (!anyOpen) $('backdrop').classList.add('hidden');
}
function closeAllLayers() {
  ['cart-drawer', 'customization-modal', 'login-modal', 'product-modal'].forEach((id) => $(id).classList.add('hidden'));
...
function saveProducts() {
  localStorage.setItem('kiwi-products', JSON.stringify(products));
}
...
function bindEvents() {
  ...
  $('admin-button').addEventListener('click', () => {
    if (localStorage.getItem('kiwi_admin_session') === 'active') showAdmin();
  ...
  $('save-menu').addEventListener('click', () => { saveProducts(); showToast('Menú guardado en este dispositivo.'); });
